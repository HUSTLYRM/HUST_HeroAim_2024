...
YOLOv10Detector::YOLOv10Detector(const std::string &model_path, const cv::Size model_input_shape, const float &model_confidence_threshold) {
...
void YOLOv10Detector::InitialModel(const std::string &model_path) {
    ov::Core core;
...
std::vector<Rect2f> YOLOv10Detector::RunInference(const cv::Mat &frame) {
    Preprocessing(frame);
...
void YOLOv10Detector::Preprocessing(const cv::Mat &frame) {
...
void YOLOv10Detector::PostProcessing() {
    ...
    */
    ...
    for (unsigned int i = 0; i < model_output_shape_.height; ++i) {
        ...
        if (confidence > model_confidence_threshold_) {
            ...
            result.box = GetBoundingBox(cv::Rect2f(x, y, w, h));
            detections_.push_back(result);
            rois_.push_back(result.box);
            cout<<result.box<<endl;
        }
    }
}

cv::Rect2f YOLOv10Detector::GetBoundingBox(const cv::Rect2f &src) const {
    cv::Rect2f box = src;
